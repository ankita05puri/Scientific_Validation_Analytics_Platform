...
def build_full_pdf(package: ValidationPackage) -> bytes:
    ...
    report_version = str(package.project_metadata.get("Report Version", "v1.0.0"))
    ...
    for index, study in enumerate(package.studies, start=6):
        ...
        traceability = pd.DataFrame(
            [
                ...
                {"Element": "Lifecycle Status", "Value": study.status},
    ...
    pdf.set_font("Arial", "", 11)
    prepared_by = str(package.project_metadata.get("Package Generated By", package.project_metadata.get("Analyst", "")) or "")
    reviewed_by = str(package.project_metadata.get("Reviewer", "") or "")
    for label in [f"Prepared By: {prepared_by}", f"Reviewed By: {reviewed_by}", "Approved By:", "Approval Date:"]:
        pdf.ln(8)
